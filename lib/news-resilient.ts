...
function decodeEntities(value = '') {
  const named: Record<string, string> = { amp: '&', quot: '"', apos: "'", lt: '<', gt: '>', nbsp: ' ', laquo: '«', raquo: '»' };
...
function cleanText(value = '') {
  return decodeEntities(value).replace(/^\s*<!\[CDATA\[/, '').replace(/\]\]>\s*$/, '').replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
}

function readTag(block: string, tag: string) {
  return cleanText(block.match(new RegExp(`<${tag}(?:\\s[^>]*)?>([\\s\\S]*?)<\\/${tag}>`, 'i'))?.[1] || '');
}
...
function readSource(block: string) {
  ...
  return {
    name: cleanText(match?.[2] || '') || 'أخبار Google',
    sourceUrl: decodeEntities(urlMatch?.[1] || 'https://news.google.com/'),
  };
}
...
function normalizeArabic(value: string) {
  return value.toLowerCase().replace(/[أإآ]/g, 'ا').replace(/ى/g, 'ي').replace(/ة/g, 'ه').replace(/ؤ/g, 'و').replace(/ئ/g, 'ي').replace(/ـ/g, '').replace(/[^\p{L}\p{N}\s]/gu, ' ').replace(/\s+/g, ' ').trim();
}
...
function detectVillage(value: string) {
  const text = normalizeArabic(value);
  const names = ['جزيرة أولاد حمزة', 'أولاد حمزة', 'الرشايدة', 'الأحايوة غرب', 'النويرات', 'عوامر العسيرات', 'أولاد جبارة', 'المساعيد', 'أولاد بهيج'];
  return names.find((name) => text.includes(normalizeArabic(name))) || 'مركز العسيرات';
}
...
async function fetchFallback(url: string) {
  const response = await fetch(url, {
    ...
    next: { revalidate: 1800, tags: ['local-news'] },
